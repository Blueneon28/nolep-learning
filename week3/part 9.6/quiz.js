function changeMe(arr) {
    ...
    if (arr.length > 0) {
        let result = {}
        arr.map(item => {
            const age = item[3] ? currentYear - item[3] : 'Invalid Birth Year'
            result.firstName = item[0]
            result.lastName = item[1]
            result.gender = item[2]
            result.age = age

            const fullName = `${result.firstName} ${result.lastName}`
            return console.log(`${fullName} :`, result);
        })
    } else {
        return console.log("");
    }
...
function shoppingTime(memberId, money) {
    ...
    if (memberId) {
        if (money > 50000) {
            ...
            const productsSortedByPrice = productSale.sort((a, b) => b.price - a.price);
            productsSortedByPrice.map(item => {
                if (changeMoney >= item.price) {
                    arrProduct.push(`${item.product}${item.brand ? ` ${item.brand}` : ''}`)
                    changeMoney = changeMoney - item.price
                }
            })

            result.memberId = memberId
            result.money = money
            result.listPurchased = arrProduct
    ...
    if (shoppers.length > 0) {
        listBarang.map(barang => {
            ...
            shoppers.map(shopper => {
                if (shopper.amount <= stock && shopper.product === barang[0]) {
                    shopperList.push(shopper.name)
                    stock = stock - shopper.amount
                    totalProfit += shopper.amount * barang[1]
                }
            })

            purchasingData.product = barang[0]
            purchasingData.shoppers = shopperList
            purchasingData.leftOver = stock
            purchasingData.totalProfit = totalProfit
            result.push(purchasingData)
        })
    }
