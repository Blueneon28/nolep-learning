...
function deepSum(arr) {
    ...
    return result > 0 ? result : 'No number'
}
...
function naikAngkot(arrPenumpang) {
    ...
    arrPenumpang.map(penumpang => {
        let passengerData = {}
        const startRoute = penumpang[1]
        const finishRoute = penumpang[2]
        const bill = (rute.indexOf(finishRoute) - rute.indexOf(startRoute)) * 2000

        passengerData.penumpang = penumpang[0]
        passengerData.naikDari = startRoute
        passengerData.tujuan = finishRoute
        passengerData.bayar = bill
