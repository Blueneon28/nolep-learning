...
function targetTerdekat(arr) {
    ...
    const indexOfO = arr.indexOf('o');
    let distance = arr.length;

    if (indexOfO === -1 || arr.indexOf('x') === -1) {
        return 0;
    }

    for (let i = 0; i < arr.length; i++) {
        if (arr[i] === 'x') {
            let distanceX = Math.abs(i - indexOfO);
...
function groupAnimals(animals) {
    // you can only write your code here!
    const uniqueLetters = new Set(animals.map(animal => animal[0]));
    let result = [];

    uniqueLetters.forEach(letter => {
        const group = animals.filter(animal => animal[0] === letter);
        result.push(group);
    });
